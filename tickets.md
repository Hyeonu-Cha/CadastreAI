...
- [x] **Task 5.16** — Surface chunk publication `date` (and `regime` when present) in `_summarise_chunks_for_synth` and the reflector's `_summarise_chunks` evidence blocks (F-5). *`regime` on chunks lands with Task 5.09; the block shows it as soon as it exists. Covered by `tests/test_synthesize.py`.*
- [ ] **Task 5.17** — Amend `_REFLECTOR_SYSTEM` to admit "all retrieved evidence predates a known regime change" as a nameable gap (F-5)
- [x] **Task 5.18** — Add a `temporal_currency` guardrail category to `src/agent/guardrails.py` with a new `annotate` action (preamble injection, not refusal). `guardrail_screen` stamps `currency_notice` on state without blocking (routing unchanged — only `blocked` ends the graph); `synthesize` prepends it deterministically. Scoped to negative gearing / CGT (not state taxes). Covered by `tests/test_guardrails.py` + `tests/test_synthesize.py`. (F-7)
